...
<?php
    ...
    //KEY FIGURES GET AS OF DATES
    function getKeyFigureAsOfDate_Displacement($datas){
        $locations = array();
        $KeyFigureLastDates = array();


        foreach ($datas as $data){
            if (!in_array($data->dis_admin0_pcode, $locations))
            {
                array_push($locations,$data->dis_admin0_pcode);
            }
        }
        ...
        foreach ($locations as $location){
            ...
            foreach ($datas as $data){
                if ($location==$data->dis_admin0_pcode) {
                    if ($data->dis_type == "IDP") {
                        if($data->dis_date> $lastDateIdp){
                            $lastDateIdp = $data->dis_date;
                        }
                    }
    
                    if ($data->dis_type == "Refugee") {
                        if($data->dis_date > $lastDateRef){
                            $lastDateRef = $data->dis_date;
                        }
                    }
    
                    if ($data->dis_type == "Returnee") {
                        if($data->dis_date > $lastDateRet){
                            $lastDateRet = $data->dis_date;
                        }
                    }
                }
            }
            array_push($KeyFigureLastDates,array("admin0"=>$location,"lastDateIdp"=>$lastDateIdp,"lastDateRef"=>$lastDateRef,"lastDateRet"=>$lastDateRet));
        }
        return $KeyFigureLastDates;
    }
    function getKeyFigureAsOfDate_Nutrition($datas){
        $locations = array();
        $KeyFigureLastDates = array();


        foreach ($datas as $data){
            if (!in_array($data->nut_admin0_pcode, $locations))
            {
                array_push($locations,$data->nut_admin0_pcode);
            }
        }

            
        foreach ($locations as $location){
            $lastDateSAM= "1900-01-01";

            foreach ($datas as $data){
                if ($location==$data->nut_admin0_pcode) {
                    if($data->nut_date> $lastDateSAM){
                        $lastDateSAM = $data->nut_date;
                    }
                }
            }
            array_push($KeyFigureLastDates,array("admin0"=>$location,"lastDateSAM"=>$lastDateSAM));
        }
        return $KeyFigureLastDates;
    }
    ...
    $keyFigureAsOfDate_Displacement = array();
    ...
    $keyFigureAsOfDate_Nutrition = array();
    ...
    $keyFigureAsOfDate_Nutrition = getKeyFigureAsOfDate_Nutrition($nutritions);



    
    foreach ($keyFigureAsOfDate_Nutrition as $keyFigureAsOfDate){
        foreach ($nutritions as $nutrition){
            ...
            if($keyFigureAsOfDate["admin0"] == $nutrition->nut_admin0_pcode){
                //traitement trend
                if($adminName !=null){
                    ...
                    if(array_key_exists($adminName,$KeyFigurenutritionsByAdmin)){
                        if ($nutrition->nut_date==$keyFigureAsOfDate["lastDateSAM"]) {
    ...
    $keyFigureAsOfDate_Displacement = getKeyFigureAsOfDate_Displacement($displacements);
    ...
    foreach ($keyFigureAsOfDate_Displacement as $keyFigureAsOfDate){
        ...
        foreach ($displacements as $displacement){
            ...
            if($keyFigureAsOfDate["admin0"] == $displacement->dis_admin0_pcode){
                    if(array_key_exists($adminName,$KeyFigureDisplacementsByAdmin)){
                        ...
                        switch ($displacement->dis_type) {
                            case 'IDP':
                                ...
                                if ($displacement->dis_date==$keyFigureAsOfDate["lastDateIdp"]) {
                            ...
                            case 'Refugee':
                                ...
                                if ($displacement->dis_date==$keyFigureAsOfDate["lastDateRef"]) {
                                    $KeyFigureDisplacementsByAdmin[$adminName] = array(
                                        "adminName"=>$adminName,
                                        "adminPcode"=>$adminPcode,
                                        "admin0"=>$displacement->dis_country,
                            ...
                            case 'Returnee':
                                ...
                                if ($displacement->dis_date==$keyFigureAsOfDate["lastDateRet"]) {
                                    $KeyFigureDisplacementsByAdmin[$adminName] = array(
                                        "adminName"=>$adminName,
                                        "adminPcode"=>$adminPcode,
                                        "admin0"=>$displacement->dis_country,
